...
public class CommandReceiver {
    ...
    public Message removeHead() throws IOException, InterruptedException, ClassNotFoundException {
        //sender.sendObject(new SerializedArgumentCommand(new RemoveByID(), ID));
        ////Thread.sleep(delay);
        return new Message(new RemoveHead());
    }
    ...
    public Message averageOfAlbumCount() throws IOException, InterruptedException, ClassNotFoundException {
        return new Message(new AverageOfAlbumCount());
    }

    public Message head() throws IOException, ClassNotFoundException, InterruptedException {
        return new Message(new Head());
    }
    ...
    public Message executeScript(String path) {
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(path))) {
            while ((line = bufferedReader.readLine()) != null) {
                if (line.split(" ")[0].matches("add|update")) {
                    ...
                    Label label = elementCreator.createScriptLabel(parameters);
                    if (label != null) {
        ...
        return new Message(new ExecuteScript());

    }

    public Message countGreaterThanNumberOfParticipants(String  NoP) {
        return new Message(new CountGreaterThanNumberOfParticipants(),NoP);
    }

    public Message addIfMax() {
        return new Message(new AddIfMax(), elementCreator.createMusicBand());
    }
